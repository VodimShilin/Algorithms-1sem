...
struct Vector_ {
	...
	int signOfRotation(const Vector_& right) const { //против часовой +
		int sign = 0;
		if (x() == 0) {
...
protected:
	mutable std::vector<Vector_> sides; //вектор векторов сторон 0->1, 1->2, 2->3 ... n->0
	mutable std::vector<double> sides_length; //вектор длин сторон 0->1, 1->2, 2->3 ... n->0
